...
<head>
  ...
  <title>Forgot Password — AITS</title>
  ...
  <style>
    ...
    *, *::before, *::after { box-sizing: border-box; }
    ...
    p.sub { font-size: 13px; color: var(--text-muted); margin-bottom: 28px; }
    label {
      display: block;
    ...
    .btn-login {
      width: 100%; background: var(--brand-accent); color: #fff;
    ...
    .alert-error {
      background: rgba(218,54,51,.15); border: 1px solid rgba(218,54,51,.2);
      color: #f85149; border-radius: 8px; padding: 10px 14px;
      font-size: 13px; margin-bottom: 20px;
      display: flex; align-items: center; gap: 8px;
    }
    .alert-success {
      background: rgba(46,160,67,.15); border: 1px solid rgba(46,160,67,.2);
      color: #3fb950; border-radius: 8px; padding: 10px 14px;
      font-size: 13px; margin-bottom: 20px;
      display: flex; align-items: center; gap: 8px;
    }
  </style>
  ...
  <form action="<?= base_url('admin/forgot-password') ?>" method="POST">
    ...
    <div class="input-wrap">
      ...
      <input type="email" id="email" name="email"
             placeholder="admin@aits.com" required />
    </div>
